...
const PersonDetailDialog = ({
  ...
  [...detailData].reverse().forEach((item) => {
    accumulatedAmount += item.amount;
    runningTotals.set(item.id, accumulatedAmount);
  });
        ...
        <Table>
          <TableHead>
            <TableRow>
              ...
              <TableCell align="right"><b>Total acumulado</b></TableCell>
          ...
          <TableBody>
            ...
            {detailData.length === 0 ? (
              <TableRow>
                <TableCell colSpan={6} align="center">No hay datos en la tabla</TableCell>
              </TableRow>
            ) : (
              detailData.map((item) => (
                <TableRow key={item.id}>
                  ...
                  <TableCell align="right" style={{ color: runningTotals.get(item.id) >= 0 ? 'green' : 'red' }}>
                    {formatAmount(runningTotals.get(item.id))}
                  </TableCell>
